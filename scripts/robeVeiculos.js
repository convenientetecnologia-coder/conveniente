...
// distribui budget (30..90s por padrão) em timers ANTES das ações do formulário.
...
async function waitForCreateItemReady(page, { timeout = 3500 } = {}) {
  ...
  async function check() {
    ...
    return await page.evaluate(() => {
      ...
      // Ajuste para veículos: "Tipo de veículo" ou "Ano"
      const tipoLbl = Array.from(document.querySelectorAll('label[role="combobox"] span')).find(s => {
        const t = (s.textContent || '').toLowerCase();
        return t.includes('tipo de veículo') || t.includes('tipo de veiculo');
      });
      const anoLbl = Array.from(document.querySelectorAll('label[role="combobox"] span')).find(s => (s.textContent || '').trim() === 'Ano');
      return !!file && (!!titulo || !!catLbl || !!tipoLbl || !!anoLbl);
    });
  }
  while ((Date.now() - start) < timeout) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          logger.info('[ROBE_V][proxy_resolved]', {
            nome,
            attempt,
...
/**
 * Helpers VEÍCULOS — Campos exclusivos da criação de veículos:
 * - Tipo de veículo: tenta digitar "Outro" + Enter; fallback: ArrowDown 8x + Enter
 * - Ano: random [anoAtual-10 .. anoAtual], digita + Enter; fallback: ArrowDown 0..12x
 * - Fabricante / Modelo: via dados/fabricante.json e dados/modelo.json
 * - Preço: random 1..3000
 */

// Utilitário: normaliza strings
function normText(s) {
  try { return String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().toLowerCase(); }
  catch { return String(s||'').trim().toLowerCase(); }
}
...
async function tryTypeInLabeledInput(page, labelText, value, { pressEnter = true, delayRange = [ROBE_TYPE_DELAY_MIN_MS, ROBE_TYPE_DELAY_MAX_MS] } = {}) {
  ...
  if (!input) {
    input = await findInputByLabel(page, labelText, 1500);
  }
  if (!input) return false;
  try { await input.click({ clickCount: 3 }); } catch {}
  await sleep(jitter(80, 160));
  await input.type(value, { delay: jitter(delayRange[0], delayRange[1]) }).catch(()=>{});
  await sleep(jitter(250, 420));
  if (pressEnter) {
...
// Tenta selecionar via combobox label: digita desired + Enter
async function trySelectComboboxByTyping(page, labelText, desiredText, { fallbackArrowTimes = 0 } = {}) {
  const combo = await findComboboxByLabel(page, labelText, 3500);
  ...
  if (fallbackArrowTimes > 0) {
    // Teste se seleção está refletida; se não, tenta fallback ArrowDown N vezes e Enter
    const maybeOk = await getComboboxSelectedValue(page, labelText);
    if (!maybeOk || !normText(maybeOk).includes(normText(desiredText))) {
      try { await combo.click(); } catch {}
      await sleep(jitter(180, 260));
      for (let i = 0; i < fallbackArrowTimes; i++) {
        try { await page.keyboard.press('ArrowDown'); } catch {}
        await sleep(60);
      }
      try { await page.keyboard.press('Enter'); } catch {}
      await sleep(jitter(200, 360));
    }
  }
  return true;
}

// Lê valor visível selecionado de um combobox pelo rótulo (best-effort)
async function getComboboxSelectedValue(page, labelText) {
  const v = await page.evaluate((labelText) => {
    const norm = (s)=> (s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
    const labs = Array.from(document.querySelectorAll('label[role="combobox"]'));
    const lab = labs.find(l => norm(l.innerText||'').includes(norm(labelText)));
    if (!lab) return null;
    // tenta encontrar badge/slot de valor selecionado dentro do label
    const candidates = Array.from(lab.querySelectorAll('span,div')).map(el => (el.innerText || el.textContent || '').trim()).filter(Boolean);
    // heurística: retorna o primeiro texto que não seja o próprio label (que contém labelText)
    const pick = candidates.find(t => !norm(t).includes(norm(labelText)) && t.length <= 60);
    return pick || null;
  }, labelText).catch(()=>null);
  return v;
}

// Seleciona "Tipo de veículo" => "Outro"
async function selecionarTipoVeiculo(page) {
  const label = 'Tipo de veículo';
  // 1) Tenta input direto
  let ok = await tryTypeInLabeledInput(page, label, 'Outro', { pressEnter: true });
  if (!ok) {
    // 2) Tenta combobox + digitar 'Outro'
    ok = await trySelectComboboxByTyping(page, label, 'Outro', { fallbackArrowTimes: 0 });
  }
  if (!ok) {
    // 3) Fallback final: abre combobox e ArrowDown 8x + Enter
    const combo = await findComboboxByLabel(page, label, 3000);
    if (combo) {
      await combo.click();
      await sleep(jitter(180, 300));
      for (let i = 0; i < 8; i++) {
  ...
  if (!ok) throw new Error('Não foi possível definir "Tipo de veículo".');
  return 'Outro';
}

// Seleciona Ano (random atual-10 até atual)
async function selecionarAnoVeiculo(page) {
  const label = 'Ano';
  const anoAtual = new Date().getFullYear();
  const alvo = String(anoAtual - Math.floor(Math.random() * 11)); // 0..10 anos atrás
  // 1) Input direto
  let ok = await tryTypeInLabeledInput(page, label, alvo, { pressEnter: true });
  if (!ok) {
    // 2) Combobox por digitação
    ok = await trySelectComboboxByTyping(page, label, alvo, { fallbackArrowTimes: 0 });
  }
  if (!ok) {
    // 3) Fallback: ArrowDown 0..12x e Enter
    const combo = await findComboboxByLabel(page, label, 3000);
    if (combo) {
      await combo.click();
      await sleep(jitter(180, 300));
  ...
  if (!ok) throw new Error('Não foi possível definir o "Ano" do veículo.');
  return alvo;
}
...
// Preço aleatório 1..3000
async function preencherPrecoVeiculo(page, valorInt) {
  const val = String(valorInt);
  const inp = await findInputByLabel(page, 'Preço', 7000);
  if (!inp) throw new Error('Campo Preço não localizado.');
  await inp.click({ clickCount: 3 });
  await sleep(jitter(120, 220));
  await inp.type(val, { delay: jitter(ROBE_PRICE_TYPE_DELAY_MIN_MS, ROBE_PRICE_TYPE_DELAY_MAX_MS) });
  await sleep(jitter(100, 180));
  try { await inp.press('Enter'); } catch {}
  await sleep(jitter(200, 320));
...
async function preencherDescricaoVeiculo(page, modeloKey) {
  ...
  await sleep(jitter(100, 180));
  await humanTypeText(page, tx, descricao, {
    minDelayMs: ROBE_DESC_TYPE_DELAY_MIN_MS,
    maxDelayMs: ROBE_DESC_TYPE_DELAY_MAX_MS,
    allowTypo: true,
    typoChance: 0.2
 ...
 * - Abre a página /marketplace/create/vehicle
 * - Preenche: Tipo de veículo, Ano, Fabricante, Modelo, Preço, Localização
 * - Mantém todos os guards de overlay, cooldown, logging e atomicidade!
 */
async function startRobe(browser, nome, robePauseMs = 0, workingNames = []) {
  ...
  try {
    ...
    stepLogArr.push(`[${nome}] Tela de criar veículo pronta (fast-lane)`);
    ...
    */
    ...
    await waitBeforeComposeAction(composePlan, 'before_vehicle_type', { nome, attId });
    ...
    await waitBeforeComposeAction(composePlan, 'before_vehicle_year', { nome, attId });
    ...
    await waitBeforeComposeAction(composePlan, 'before_make_model', { nome, attId });
    ...
    if (!published) {
      ...
      logger.warn('[ROBE] Publicação de veículo não confirmada', { nome, titulo });
    ...
    if (published) {
      try {
        if (fotoNome) {
          ...
          await fotosV.markPostedAndMaybeDelete(nome, fotoNome, allWorkingProfiles);
        }
      } catch (e) {
    ...
    try { await logIssue(nome, 'robe_success', 'Publicação de veículo concluída com sucesso.'); } catch {}
  ...
  // GARANTE FAIL-CLOSED: NUNCA DUPLICA PARA A MESMA CONTA!
    ...
    logger.error('Erro em fluxo Robe (veículos)', { nome }, e);
