...
const api = {
  ...
  closeAll:        () => fetch('/api/perfis/close-all', { method: 'POST' }).then(r => r.json()),
  recyclePerfil:   (nome) => fetch(`/api/perfis/${encodeURIComponent(nome)}/recycle`, { method:'POST' }).then(r=>r.json()),
  recycleAll:      () => fetch('/api/perfis/recycle-all', { method:'POST' }).then(r=>r.json()),
