...
// ==== INÍCIO DAS NOVAS APIS OBRIGATÓRIAS ====
getUaPresets: () => fetch('/api/ua-presets').then(r => r.json()),
patchCidade: (nome, novaCidade) => fetch(`/api/perfis/${encodeURIComponent(nome)}/cidade`, {
    method:'PATCH',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ novaCidade })
}).then(r=>r.json()),
patchUa: (nome, body) => fetch(`/api/perfis/${encodeURIComponent(nome)}/ua`, {
    method:'PATCH',
    headers:{'Content-Type':'application/json'},
