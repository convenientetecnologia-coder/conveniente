...
| INC-20260201-0000-01 | `docs/inbox/in_progress/INC-20260201-0000-01.md` | P1 | conveniente+sitechatbot | Garantir que todo host atualizado/reiniciado baixa automaticamente `groq_config.json` com modelo correto (sem intervenção) | Host precisa sinalizar needsGroqConfig também em caso de mismatch do modelo; CT envia set_groq_config idempotente | `/report` payload.groq + commands.log/ack set_groq_config | in_progress | needs_restart | not_run | não | sim |
