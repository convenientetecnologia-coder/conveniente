...
| INC-20260202-1600-01 | `docs/inbox/done/INC-20260202-1600-01.md` | P0 | sitechatbot+conveniente+notificador | Cidades/Grupos: contrato canônico + prioridade de provisão (estoque→servidor) + migração manual | Fixar contrato: CT canônico=`cidade_uf`; `conveniente` recebe `cidade` sem UF; `notificador` depende de `cidade_uf`; depois construir score único (24/48/72h + motoristas + A + LE por idade ~12d + warmup 24h) | evidência por código: `C:\\sitechatbot\\index.js`, `C:\\conveniente\\scripts\\dashboard.js`, `C:\\notificador\\index.js` | done | deployed | passed | não | não |
| INC-20260202-2000-01 | `docs/inbox/done/INC-20260202-2000-01.md` | P0 | sitechatbot+conveniente | Fonte Única da Verdade: Virtus→Grupos vs Contas FB v2 divergindo (janelas + classificação A/LR/LE/B) | Hoje existiam “2 verdades”: dashboard usava recent3d + classificador simples; v2 usava `fbAccountState`. Unificado para agregador canônico + janelas explícitas + includeOffline explícito | evidência: CT `C:\\sitechatbot\\index.js` (`/api/dashboard/virtus`, `/api/contas-facebook`, `computeAccountsByGroupFromSnapshots`) + verificador offline `C:\\sitechatbot\\tools\\verify_virtus_groups_truth.js` | done | deployed | passed | não | não |
...
6) **Organização por pastas (status físico)**
   ...
   - Ao marcar `need_evidence`: mover para `docs/inbox/need_evidence/INC-...md` (aguardando evidência/decisão; não é WIP ativo)
