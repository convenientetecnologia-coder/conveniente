...
module.exports = (app, workerClient, fileStore) => {
  ...
  // Fechar TODOS (enterprise): 1 endpoint server-side que faz desired + broadcast deactivate-all
  // POST /api/perfis/:nome/recycle — reciclar um perfil específico
  app.post('/api/perfis/:nome/recycle', async (req, res) => {
    const nome = req.params.nome;
    try {
      assertPerfilExists(fileStore, nome);
      const r = await workerClient.sendWorkerCommand('recycle', { nome, reason: 'admin_manual' }, { timeoutMs: 600000 });
      return res.json(r);
    } catch (e) {
      logger.error('Erro ao reciclar perfil', { rota: '/api/perfis/:nome/recycle', nome, error: e && e.message }, e);
      return res.json({ ok: false, error: (e && e.message) || String(e) });
    }
  });

  // POST /api/perfis/recycle-all — reciclar todos os perfis
  app.post('/api/perfis/recycle-all', async (req, res) => {
    logger.info('POST /api/perfis/recycle-all chamada', {});
    try {
      try { await issues.append('system', 'mil_action', 'admin_recycle_all_requested').catch(()=>{}); } catch {}
      const r = await workerClient.sendWorkerCommand('recycle-all', { reason: 'admin_manual_all' }, { timeoutMs: 60000 });
      return res.json(r);
    } catch (e) {
      logger.error('Erro ao reciclar todos os perfis', { rota: '/api/perfis/recycle-all', error: e && e.message }, e);
      return res.json({ ok: false, error: (e && e.message) || String(e) });
    }
  });

  app.post('/api/perfis/close-all', async (req, res) => {
    logger.info('POST /api/perfis/close-all chamada', {});
    try { await issues.append('system', 'mil_action', 'admin_close_all_requested'); } catch {}
    let perfis = [];
    try { perfis = fileStore.loadPerfisJson() || []; } catch { perfis = []; }
    try {
      await fileStore.withDesiredFileLockUpdate((desired) => {
        desired.perfis = desired.perfis || {};
        for (const p of perfis) {
          if (!p || !p.nome) continue;
          const prev = desired.perfis[p.nome] || {};
          desired.perfis[p.nome] = {
            ...prev,
            active: false,
            virtus: 'off',
            invokeHuman: false,
            robePlay: false,
            configureOnce: false
            // IMPORTANTE: não mexer em humanHold
          };
        }
        return desired;
      });
    } catch (e) {
      logger.error('Erro ao patchDesired durante close-all', { rota: '/api/perfis/close-all', error: e && e.message }, e);
    }

    try {
      const resp = await workerClient.sendWorkerCommand('deactivate-all', { reason: 'admin_close_all' }, { timeoutMs: 600000 });
      if (!resp) return res.json({ ok: false, error: 'close_all_no_response' });
      return res.json({ ok: true, ...resp });
    } catch (e) {
      logger.error('Erro ao enviar comando deactivate-all ao cluster', { rota: '/api/perfis/close-all', error: e && e.message }, e);
      return res.json({ ok: false, error: (e && e.message) || 'close_all_failed' });
    }
  });
