...
| INC-20260201-0300-01 | `docs/inbox/in_progress/INC-20260201-0300-01.md` | P0 | conveniente+sitechatbot | Total>ativos: browsers fechados apesar de RAM; prejuízo (contas paradas) | `desired.active` pode estar sendo desligado automaticamente (ex.: open_all partial) ou algum gate/backoff está segurando sem motivo | CT snapshots `C:\\sitechatbot\\dados\\<hostId>-*.json` + fetch_logs(keys=[desired,status,provision_audit]) RM1/RM2/RM4/RM6 | in_progress | not_deployed | not_run | não | sim |
