...
const allowSrc = fs.readFileSync(path.join(root, "scripts", "logsAllowlist.js"), "utf8");
const coreSrc = fs.readFileSync(path.join(root, "scripts", "logsFetchCore.js"), "utf8");
const fetchExecSrc = fs.readFileSync(path.join(root, "scripts", "logFetchExec.js"), "utf8");
check("src_dash_allow_prev", allowSrc.includes("index_lifecycle_prev") && allowSrc.includes("index_handle_pulse"));
check("src_dash_allow_host_exit", dashSrc.includes("index_host_exit") || allowSrc.includes("index_host_exit:"));
check("src_allow_crash_nativo", allowSrc.includes("crash_nativo_index") && allowSrc.includes("cell_${i}_native"));
