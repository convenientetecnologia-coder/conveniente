...
async function tryFixPhantom(nome, page) {
  ...
  await deactivate({ nome, reason: 'phantom_reopen', policy: 'preserveDesired' });
  setKillGuard(nome);
  ph.lastActionAt = now;
  return true;
}
...
try { log.info('[WORKER][BOOT] log error', e && e.message || e); } catch {}
}
...
try {
  ...
  try { log.warn('[BOOT] Erro ao repopular frozenUntil dos manifests:', err && err.message || err); } catch {}
}
...
async function activateOnce(nome, source = '') {
  ...
  try {
    ...
    const job = (async () => {
      try {
        log.info('[WORKER][activateOnce] start nome=' + nome + ' source=' + source);
        ...
        //     try { await deactivate({ nome, reason: 'open_headroom', policy: 'preserveDesired' }); } catch {}
        //     if (_supervisorSlotGranted) { try { await supervisorClient.notifyOpened(nome, 'err'); } catch {} }
        ...
        log.info('[WORKER][activateOnce] done nome=' + nome + ' source=' + source);
        ...
        // FIM DA INSTRUÇÃO 9
        log.warn('[WORKER][activateOnce] fail nome=' + nome + ' source=' + source + ':', e && e.message || e);
        if (_supervisorSlotGranted) { try { await supervisorClient.notifyOpened(nome, 'err'); } catch {} }
...
async function closeExtraPages(browser, mainPage, nome) {
  try {
    ...
    if (closed > 0) {
      log.info('[PRUNER] Fechou ' + closed + ' abas extras.');
    } else if (process.env.PRUNE_DEBUG === '1') {
      log.debug('[PRUNER] Nada a fechar.');
    }
  } catch (e) {
    if (process.env.PRUNE_DEBUG === '1') {
      log.warn('[PRUNER] Erro prune:', e && e.message || e);
    }
  }
}
...
async function ramCpuMonitorTick() {
  ...
  try {
    ...
    if (process.env.METRICS_DEBUG === '1') {
      log.debug('[METRICS] pidsByNome:', Object.keys(pidsByNome), 'Example command:', Object.values(pidsMeta)[0]?.cmd || '');
    }
      ...
      promises.push((async () => {
        ...
        if (hist.length >= 5) {
          ...
          if (allHigh) {
            ...
            await deactivate({nome, reason:'cpuKill', policy:'preserveDesired'});
            setKillGuard(nome);
  ...
  for (const nome of Object.keys(robeMeta)) {
    ...
    if (hist.length >= 5) {
      ...
      if (allHigh || slopeOK) {
        ...
        await deactivate({ nome, reason:'ramKill', policy:'preserveDesired' });
        setKillGuard(nome);
...
async function robeTickGlobal() {
  log.info('[WORKER][robeTickGlobal] Tick fila global, hora: ' + new Date().toLocaleString());
  ...
  for (const nome of prontos) {
    ...
    log.info(`[WORKER][robeTickGlobal] Enfileirando ${nome}? cooldown=${await normalizeCooldown(nome)}s inQueue=${robeQueue.inQueue(nome)}, isActive=${robeQueue.isActive(nome)}`);
    ...
    robeQueue.enqueue(nome, async () => {
      ...
      try { log.info(`[WORKER][robeTickGlobal] Robe start: ${nome}`); } catch {}
      ...
      try {
        ...
        try {
          ...
          try { log.warn('[WORKER][robeTickGlobal] Robe error:', e && e.message || e); } catch {}
          return;
        }
        ...
        if (res && res.ok) {
          ...
          try { log.info(`[WORKER][robeTickGlobal] Robe success: ${nome}`); } catch {}
        } else {
          robeUpdateMeta(nome, {
            estado: 'idle',
            cooldownSec: await normalizeCooldown(nome)
        ...
        try { log.info(`[WORKER][robeTickGlobal] Robe end: ${nome}`); } catch {}
      }
    });
...
async function fotosGcTick() {
  try {
    ...
    if (res && (res.deletedFiles || res.removedIndex || res.resetGens)) {
      log.info(`[FOTOS][GC] deletedFiles=${res.deletedFiles} removedIndex=${res.removedIndex} resetGens=${res.resetGens}`);
    }
  } catch (e) {
    log.warn('[FOTOS][GC] erro:', e && e.message || e);
  }
}
...
try {
log.info(`[WORKER][BROWSER] disconnected: ${nome}`);
...
try { await snapshotStatusAndWrite(); } catch {}
  ...
  try { log.warn('[WORKER][BROWSER] disconnect handler err:', e && e.message || e); } catch {}
}
...
try { log.warn('[WORKER][statusWrite] erro:', e && e.message || e); } catch {}
}
});
...
async function ensureFrozenShutdown(nome, origin = 'frozen') {
  ...
  try {
    ...
    await deactivate({ nome, reason: 'frozen', policy: 'preserveDesired' });
  } catch {}
    ...
    for (const nome of Object.keys(desired.perfis || {})) {
      ...
      if (!pages || !pages[0]) {
        ...
        if (retryFailed) {
          ...
          if (robeMeta[nome].noPagesStrikes >= 2 && (Date.now() - robeMeta[nome].lastNoPagesAt) >= 5000) {
            ...
            await deactivate({ nome, reason: 'nurse_zombie', policy: 'preserveDesired' });
            setKillGuard(nome);
      ...
      if (det && det.blocked && det.domain === 'messenger') {
        ...
        if (robeMeta[nome].blockDetectWindow.length >= 2 && (!robeMeta[nome].blockHysteresisUntil || robeMeta[nome].blockHysteresisUntil < now2)) {
          ...
          await deactivate({ nome, reason: 'virtus_block', policy: 'preserveDesired' });
          setKillGuard(nome);
          await snapshotStatusAndWrite();
          continue;
        } else {
      ...
      if (ctrl && ctrl.configurando) {
        log.info(`[NURSE][SKIP PRUNE] Perfil ${nome} está configurando, prune ignorado.`);
        continue;
      }
...
async function trySwapOpen(target) {
  ...
  for (const cand of candidates) {
    ...
    await deactivate({ nome: cand.n, reason: 'swap_for_open', policy: 'preserveDesired' });
...
async function escalateToReopen(nome, reason='health_reopen') {
  ...
  await deactivate({ nome, reason, policy: 'preserveDesired' });
  setKillGuard(nome);
  const st = getHealth(nome);
  st.stage = 'reopen';
...
// ===== Core deactivate (ex-handler) =====
async function deactivate({ nome, reason, policy }) {
...
// ===== initTimers =====
let _timersInitialized = false;
let _intervals = [];
let _timeouts = [];

function initTimers() {
  if (_timersInitialized) return;
...
module.exports = {
  ...
  // robe/cooldown helpers
  normalizeCooldown,
  releaseLimitPostingIfExpired,
  robeCooldownLeft,
  robeLastPosted,
  robeUpdateMeta,
  getWorkingProfileNames,
  ...
  freezeProfileFor,
  unfreezeProfile,
  ensureFrozenShutdown,
  isFrozenNow,
  setKillGuard,
  killGuardActive,
  trySwapOpen,
  evaluateChatsState,
  isPhantomFromSnapshot,
  isOkFromSnapshot,
  tryFixPhantom,
  detectMessengerTempBlock,
  isLimitPostingActive,
  isLimitPostingRes,
