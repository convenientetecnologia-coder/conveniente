...
| INC-20260201-0100-01 | `docs/inbox/in_progress/INC-20260201-0100-01.md` | P1 | sitechatbot+conveniente | Menu Servidores: mostrar contagem de “Login/Cookies falhou” e “Recurso em análise” no pill do servidor; ordenar OFFLINE primeiro | CT hoje não consegue contar “login/cookies falhou” porque `status.perfis` não expõe `loginRemediateFailed`; appeal já existe; sort não prioriza offline | evidência por código: `conveniente/scripts/worker.js` (`setLoginRemediateFailedFlag`, `setAppealSubmittedFlag`, `snapshotStatusAndWrite`) + `sitechatbot/index.js` (`GET /servers flagsAgg + sort`) + `sitechatbot/public/index.html` (render pills) | in_progress | not_deployed | not_run | não | sim |
